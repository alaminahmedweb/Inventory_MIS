$(function() {
    ...
    $('#txtReceiptNo').keydown(function (e) { if (e.which === 13) { $('#txtSupplierId').focus(); } });
    $("#txtSupplierId").on("select2:close", function () { $("#ProductId").focus(); });
    ...
    $('#txtSupplierId').on('change', function () {
        ...
        $.ajax({
            type: "POST",
            url: site_url + "StockReceive/GetSupplierList/",
            contentType: "application/json; charset=utf-8",
            data: JSON.stringify(json),
            success: function(data) {
                //alert(data);
                $.each(data, function(key, value) {
                    $('#txtMobileNo').val(value.ContactNo);
                    $('#txtAddress').val(value.PartyAddress);
                });
            }
        });
    });
    ...
    $('#ProductId').on('change', function () {
        ...
        $.ajax({
            type: "POST",
            url: site_url + "StockReceive/GetProductListById/",
            contentType: "application/json; charset=utf-8",
            data: JSON.stringify(json),
            success: function(data) {
                //alert(data);
...
////Form Validation
    ...
    function save() {
        ...
        if (res == true) {
            ...
            $.ajax({
                type: "POST",
                url: site_url + "StockReceive/Save/",
                contentType: "application/json; charset=utf-8",
                data: JSON.stringify(json),
                success: function (data) {
                    alert(data);
    ...
    function validation() {
        ...
        if ($('#txtSupplierId').val() == "0") { $('.SupplierName .select2-selection').css('border-color', 'red'); isValid = false; }
        else { $('.SupplierName .select2-selection').css('border-color', 'lightgrey'); }
